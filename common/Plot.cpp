    ...
    if (ImPlot::BeginSubplots("My Subplots", rows, cols, ImVec2(-1, 400), flags, rratios, cratios)) {
        ...
        for (int i = 0; i < rows * cols; ++i) {
            if (ImPlot::BeginPlot("", ImVec2(), ImPlotFlags_NoLegend)) {
                ...
                float fi = 0.01f * (i + 1);
                if (rows * cols > 1) {
                    ImPlot::SetNextLineStyle(ImPlot::SampleColormap((float)i / (float)(rows * cols - 1), ImPlotColormap_Jet));
                }
                char label[16];
...
void Demo_LegendOptions() {
    ...
    ImGui::SliderFloat2("LegendPadding", (float*)&ImPlot::GetStyle().LegendPadding, 0.0f, 20.0f, "%.0f");
    ImGui::SliderFloat2("LegendInnerPadding", (float*)&ImPlot::GetStyle().LegendInnerPadding, 0.0f, 10.0f, "%.0f");
    ImGui::SliderFloat2("LegendSpacing", (float*)&ImPlot::GetStyle().LegendSpacing, 0.0f, 5.0f, "%.0f");
